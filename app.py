...
def check_order():
    ...
    # 正規表現のグループ詳細を表示
    match = re.search(STARBUCKS_JP_DRINK_MENU_REGEX, order_str)
    if match:
        result_text += "\n--- マッチ詳細 ---\n"
...
# GUI Setup
root = tk.Tk()
root.title("Starbucks(JP) Drink Menu Regex Checker")
root.geometry("600x450")
root.configure(bg="#f2f0eb")

frame = tk.Frame(root, padx=20, pady=20, bg="#f2f0eb")
...
entry = tk.Entry(frame, width=70, font=("Helvetica", 14))
entry.pack(pady=10)
entry.insert(0, "トール エクストラホイップ ダークモカチップフラペチーノ")

button = tk.Button(frame, text="判定する", command=check_order, font=("Helvetica", 12, "bold"), bg="#00704A", fg="white", padx=10, pady=5)
button.pack(pady=10)

result_label = tk.Label(frame, text="", justify=tk.LEFT, font=("Helvetica", 11), bg="#ffffff", anchor="nw", relief=tk.SOLID, bd=1, padx=10, pady=10)
